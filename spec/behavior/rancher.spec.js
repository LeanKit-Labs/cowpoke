...
resquestMoch.get = ( route, callback ) => {
	...
	if ( route === "http://myrancher.com/v1" ) {
		mock = { auth: () => callback( null, { body: JSON.stringify( rancherEnv ) } ) };
	} else if ( route === "http://myrancher.com/v1/projects" ) {
		mock = { auth: () => callback( null, { body: JSON.stringify( { data: environmentsBody } ) } ) };
	} else if ( route === "http://myrancher.com/v1/projects/l0l/environments" ) {
		mock = { auth: () => callback( null, { body: JSON.stringify( { data: stacksBody } ) } ) };
	} else if ( route === "http://myrancher.com/v1/projects/l0l/environments/2a2" ) {
		mock = { auth: () => callback( null, { body: JSON.stringify( { data: newStack } ) } ) };
	} else {
		mock = { auth: () => callback( "err" ) };
	}
...
describe( "Rancher API", () => {
	let rancher, environments, stacks;
	before( () => {
